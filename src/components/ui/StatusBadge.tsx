...
export function EvidenceStatusBadge({ status }: { status: EvidenceStatus }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-full py-1 pl-2 pr-2.5 text-xs font-semibold ${style.pill}`}
    >
...
export function ComplianceResultBadge({ result }: { result: ComplianceResult }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-md border-[1.5px] bg-white px-2.5 py-1 text-xs font-semibold ${style.border} ${style.text}`}
    >
